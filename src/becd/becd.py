...
def update_project(project, row):
    ...
    product = Product(
        ...
        quantity=1,
        reference_service_life=row[MAPPING["assemblies.products.reference_service_life"]],
        unit=Unit.kg,
        transport=None,
...
def get_assembly_results(row: dict):
    ...
    result_items = [(key, value) for key, value in MAPPING.items() if key.startswith("assemblies.results.") and value]
    for key, value in result_items:
        results[ImpactCategoryKey[key.split(".")[2]]][LifeCycleStage[key.split(".")[3]]] = (
            float(row[value])
        )
    return results
...
def get_thermal_envelope_area(row: dict):
    values = [
        float(row[key] or 0)
        for key in MAPPING["meta_data.thermal_envelope_area.value"]
    ]
    return {
        "value": sum(values),
        "unit": Unit.m2,
...
def add_project(row: dict):
    ...
    project = Project(
        ...
        location=get_location(row[MAPPING["location.city"]], row[MAPPING["location.country"]]),
        results=results,
        project_info=ProjectInfo(
            type="buildingInfo",
            building_completion_year=date_to_year(row[MAPPING["project_info.building_completion_year"]]),
            building_height=ValueUnit(value=float(row[MAPPING["project_info.building_height.value"]]), unit=Unit.m),
            building_footprint=ValueUnit(value=float(row[MAPPING["project_info.building_footprint.value"]]),
                                         unit=Unit.m),
            ...
            floors_above_ground=int(row[MAPPING["project_info.floors_above_ground"]]) or 0,
            floors_below_ground=row[MAPPING["project_info.floors_below_ground"]] or None,
        ...
        meta_data={
            ...
            "assessment": {
                ...
                "en15978_compliance": row[MAPPING["meta_data.assessment.en15978_compliance"]] == "Fully compliant",
                "rics_2017_compliance": row[MAPPING[
                    "meta_data.assessment.rics_2017_compliance"]] == "Fully compliant with 2017 version",
                ...
                "assessor": {
                    ...
                    "organization": row[MAPPING["meta_data.assessment.assessor.organization"]],
                },
            ...
            "cost": {
                ...
                "currency": "gbp",
            } if row[MAPPING["meta_data.cost.total_cost"]] else None,
            "demolished_area": {
                "value": float(row[MAPPING["meta_data.demolished_area.value"]]),
                "unit": Unit.m2,
            } if row[MAPPING["meta_data.demolished_area.value"]] else None,
            "newly_built_area": {
                "value": float(row[MAPPING["meta_data.newly_built_area.value"]]),
                "unit": Unit.m2,
            } if row[MAPPING["meta_data.newly_built_area.value"]] else None,
            "retrofitted_area": {
                "value": float(row[MAPPING["meta_data.retrofitted_area.value"]]),
                "unit": Unit.m2,
            } if row[MAPPING["meta_data.retrofitted_area.value"]] else None,
            "project_site_area": {
                "value": float(row[MAPPING["meta_data.project_site_area.value"]]),
                "unit": Unit.m2,
            } if row[MAPPING["meta_data.project_site_area.value"]] else None,
            "thermal_envelope_area": get_thermal_envelope_area(row),
            "structural": {
                "column_grid_long": {
                    "value": float(row[MAPPING["meta_data.structural.column_grid_long.value"]]),
                    "unit": Unit.m,
                },
                "foundation_type": row[MAPPING["meta_data.structural.foundation_type"]],
                "vertical_gravity_system": row[MAPPING["meta_data.structural.vertical_gravity_system"]],
                "secondary_vertical_gravity_system": row[
                    MAPPING["meta_data.structural.secondary_vertical_gravity_system"]],
                "horizontal_gravity_system": row[MAPPING["meta_data.structural.horizontal_gravity_system"]],
        ...
        software_info=SoftwareInfo(
            ...
            goal_and_scope_definition=row[MAPPING["software_info.goal_and_scope_definition"]]
        ),
    )
