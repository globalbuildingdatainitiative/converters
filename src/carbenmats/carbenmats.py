...
def get_roof_type(row: dict):
    ...
    _keys = [
        key for key, value in MAPPING.items() if key.startswith("roof_type.") and value
    ]
...
def convert_row(row: dict):
    ...
    project = Project(
        ...
        location=get_location(
            get_value_or_none(row, "location.country"),
            get_value_or_none(row, "location.city"),
        ),
        name=get_name(row[MAPPING["name"]]),
        project_info=ProjectInfo(
            type="buildingInfo",
            building_completion_year=get_value_or_none(
                row, "project_info.building_completion_year"
            ),
            building_users=get_users(row, "project_info.building_users"),
            gross_floor_area=AreaType(
                unit=Unit.m2,
                value=get_value_or_none(row, "project_info.gross_floor_area.value")
                or 0,
                definition=get_value_or_none(
                    row, "project_info.gross_floor_area.definition"
                )
                or "",
            ),
            building_footprint=ValueUnit(
                unit=Unit.m2,
                value=get_value_or_none(row, "project_info.building_footprint.value"),
            )
            if get_value_or_none(row, "project_info.building_footprint.value")
            else None,
            building_type=get_building_type(row),
            building_typology=get_building_typology(row),
            floors_above_ground=get_value_or_none(
                row, "project_info.floors_above_ground"
            )
            or 0,
            floors_below_ground=get_value_or_none(
                row, "project_info.floors_below_ground"
            )
            or None,
        ...
        meta_data={
            "assessment": {"year": get_year(row, "meta_data.assessment.year")},
            "source": {"name": "CarbEnMats", "url": None},
        },
        project_phase=ProjectPhase.other,
        results=results,
        software_info=SoftwareInfo(
            lca_software=get_value_or_none(row, "software_info.lca_software") or "",
            goal_and_scope_definition=get_value_or_none(
                row, "software_info.goal_and_scope_definition"
            ),
        ),
    )
